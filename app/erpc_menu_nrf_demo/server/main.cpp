 ...
 * Target: nRF52840-DK (UART link between two boards)
 */
...
public:
    ...
    CalcResult * calculate(int32_t a, int32_t b, ArithmeticOp operation) override {
        std::printf("[Server] calculate(%ld, %ld, op=%d)\n", (long)a, (long)b, (int)operation);
        ...
        switch (operation) {
            case OP_ADD:
                result->result = a + b;
                std::printf("[Server] ADD: %ld + %ld = %ld\n", (long)a, (long)b, (long)result->result);
                break;
            case OP_SUBTRACT:
                result->result = a - b;
                std::printf("[Server] SUBTRACT: %ld - %ld = %ld\n", (long)a, (long)b, (long)result->result);
                break;
            case OP_MULTIPLY:
                result->result = a * b;
                std::printf("[Server] MULTIPLY: %ld * %ld = %ld\n", (long)a, (long)b, (long)result->result);
                break;
            case OP_DIVIDE:
                if (b == 0) {
                    ...
                    std::printf("[Server] DIVIDE: %ld / %ld = %ld\n", (long)a, (long)b, (long)result->result);
                }
                break;
            default:
    ...
    IntArray * sortArray(const IntArray * input_array) override {
        std::printf("[Server] sortArray(length=%ld)\n", (long)input_array->length);
        ...
        for (uint32_t i = 0; i < input_array->data.elementsCount; i++) {
            ...
            std::printf("%ld", (long)input_array->data.elements[i]);
...
{
    std::puts("====================================");
    std::puts("  Enhanced RPC Demo - SERVER (UART)");
    std::puts("  RIOT OS + eRPC on nRF52840-DK");
    std::puts("====================================");

    // UART_DEV(1) maps to pins D0/D1 on the nRF52840-DK (connect TX<->RX, common GND)
    erpc_transport_t transport = (erpc_transport_t)erpc_uart_transport_init(UART_DEV(1), 115200);
    if (!transport) {
        std::puts("[ERROR] UART transport create failed");
        return 1;
    }
    ...
    std::puts("[Server] Ready on UART1 @115200. Waiting for client...\n");

    // run server loop
    while (1) {
        erpc_status_t st = erpc_server_run(srv);
        if (st != kErpcStatus_Success) {
            // Yield if no message was processed
            thread_yield();
        }
    }
